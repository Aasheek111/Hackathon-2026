  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-3 text-xs font-extrabold uppercase tracking-wide border-b border-sky-200/60 pb-2">
        <Hand className={`w-4 h-4 ${highContrast ? "text-yellow-300" : "text-sky-700 dark:text-sky-400"}`} />
        <span className={highContrast ? "text-yellow-300" : "text-slate-900 dark:text-white"}>
      ...
      <div className="flex flex-wrap gap-3 w-full">
        {letters.map((ch, i) => (
          ...
          >
            <SignSymbol term={ch} size={50} showMotion={false} className="drop-shadow-xs" />
      ...
      {active !== null && (
        <div className="flex items-start gap-3 mt-3 p-3.5 rounded-xl bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 shadow-xs">
          <div className={`shrink-0 rounded-lg p-1.5 ${highContrast ? "bg-black border border-yellow-400" : "bg-sky-50 dark:bg-slate-900"}`}>
            <SignSymbol term={letters[active]} size={68} />
          </div>
          <p className={`text-xs leading-relaxed ${highContrast ? "text-yellow-200" : "text-slate-800 dark:text-gray-100"}`}>
            <strong className="text-slate-900 dark:text-white font-extrabold">Letter {letters[active]}:</strong> {ASL_ALPHABET[letters[active]]}
          </p>
        </div>
      )}
    </div>
