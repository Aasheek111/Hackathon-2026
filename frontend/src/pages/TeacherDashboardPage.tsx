    ...
    return (
      <div className="min-h-screen bg-[#FAF9F5] flex items-center justify-center p-6 text-slate-800 font-sans">
        <div className="bg-white max-w-lg w-full p-8 sm:p-10 rounded-3xl border border-slate-200/80 shadow-md">
          <h1 className="text-2xl font-bold text-slate-900 mb-2">Create Your Classroom</h1>
          <p className="text-slate-600 text-sm mb-6">Every educator starts with one main classroom. You can set admission criteria later.</p>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-6xl mx-auto space-y-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">{classroom.description || 'No description provided.'}</p>
          </div>
        </div>
...
const OverviewTab: React.FC<{ classroom: Classroom }> = ({ classroom }) => (
  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
    {[
      { label: 'Students Enrolled', value: classroom.enrolments.length, color: 'bg-emerald-50 text-emerald-800' },
      { label: 'Pending Requests', value: classroom.joinRequests.length, color: 'bg-amber-50 text-amber-800' },
      { label: 'Total Subjects', value: classroom.subjects.length, color: 'bg-sky-50 text-sky-800' }
    ].map((s, i) => (
      <div key={i} className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs">
        <div className="text-slate-500 text-xs font-bold uppercase tracking-wider mb-2">{s.label}</div>
...
const ContentTab: React.FC<{ classroom: Classroom; onChanged: () => Promise<void> }> = ({ classroom, onChanged }) => {
  ...
  const statusBadge = (status: Unit['indexStatus']) => {
    ...
    return <span className={`text-[11px] font-bold px-2.5 py-0.5 rounded-full border ${map[status]}`}>{status.replace('_', ' ')}</span>;
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={addSubject} className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs flex gap-3 items-end">
        <div className="flex-1">
          <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">New Subject</label>
          <input
            type="text"
            value={subjectName}
      ...
      {classroom.subjects.map((subject) => (
        <div key={subject.id} className="bg-white rounded-3xl p-6 border border-slate-200/80 shadow-xs">
          <h3 className="text-lg font-bold text-slate-900 mb-4">{subject.name}</h3>

          <div className="space-y-3">
            {subject.units.map((unit) => (
              <div key={unit.id} className="bg-[#FAF9F5] border border-slate-200/70 rounded-2xl p-4 flex items-center justify-between gap-4 flex-wrap">
                <div className="flex items-center gap-3">
                  <FileText className="w-4 h-4 text-slate-400" />
                  <span className="font-bold text-slate-800 text-sm">{unit.title}</span>
                  {statusBadge(unit.indexStatus)}
                  <span className="text-xs text-slate-500 font-medium">{unit._count?.documents || 0} document(s)</span>
                </div>
      ...
      {classroom.subjects.length === 0 && (
        <p className="text-center text-slate-400 text-xs py-8">No subjects created yet. Add one above to start uploading syllabus PDFs!</p>
      )}
    </div>
  );
};
  ...
  return (
    <div className="space-y-4">
      {classroom.joinRequests.map((req) => (
        <div key={req.id} className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs">
          <div className="flex items-center justify-between flex-wrap gap-4 mb-4">
            ...
            <div className="flex items-center gap-4">
              <div className="text-right bg-emerald-50 border border-emerald-200 px-3 py-1.5 rounded-2xl">
                <div className="text-xl font-bold text-emerald-700">{Math.round(req.matchScore)}%</div>
                <div className="text-[10px] font-bold text-emerald-800 uppercase tracking-wider">match score</div>
              </div>
              <button onClick={() => approve(req.id)} className="p-2.5 bg-emerald-100 text-emerald-800 rounded-2xl hover:bg-emerald-200 transition-colors font-bold">
                <Check className="w-5 h-5" />
              </button>
              <button onClick={() => reject(req.id)} className="p-2.5 bg-rose-100 text-rose-800 rounded-2xl hover:bg-rose-200 transition-colors font-bold">
                <X className="w-5 h-5" />
              </button>
            </div>
          </div>
          <div className="space-y-1.5">
            {req.matchReasons.filter((r) => r.applicable).map((r, i) => (
              <div key={i} className={`text-xs font-medium flex items-center gap-2 ${r.passed ? 'text-emerald-700' : 'text-amber-700'}`}>
                {r.passed ? <Check className="w-3.5 h-3.5 text-emerald-600" /> : <X className="w-3.5 h-3.5 text-amber-600" />} {r.detail}
              </div>
            ))}
            {req.matchReasons.filter((r) => r.applicable).length === 0 && (
              <p className="text-xs text-slate-500">This classroom has no admission criteria set — every student matches by default.</p>
            )}
          </div>
        </div>
      ))}
      {classroom.joinRequests.length === 0 && <p className="text-center text-slate-400 text-xs py-8">No pending join requests.</p>}
    </div>
  );
};
    ...
    <table className="w-full text-left border-collapse">
      ...
      <tbody className="text-sm">
        {classroom.enrolments.map((e) => {
          ...
          return (
            <tr key={e.id} className="border-b border-slate-100 hover:bg-[#FAF9F5]">
              <td className="p-4">
              ...
              <td className="p-4">
                {attempt ? (
                  <span className={`font-bold ${attempt.scorePercent >= 70 ? 'text-emerald-700' : attempt.scorePercent >= 40 ? 'text-amber-700' : 'text-rose-700'}`}>
              ...
              <td className="p-4 text-slate-700 font-medium">{attempt ? `${Math.round(attempt.attentionSpanScore)}%` : '—'}</td>
              <td className="p-4 text-slate-700 font-medium">{attempt?.preferredMode || '—'}</td>
              <td className="p-4 text-slate-700 font-medium">{progress?.xp ?? 0}</td>
              <td className="p-4 text-slate-700 font-medium">{progress?.streakDays ?? 0}d</td>
              <td className="p-4 text-slate-700 font-medium">{progress?.badges.length ?? 0}</td>
              <td className="p-4 text-slate-500 text-xs font-medium">{new Date(e.joinedAt).toLocaleDateString()}</td>
            </tr>
          );
        })}
        {classroom.enrolments.length === 0 && (
          <tr><td colSpan={8} className="p-8 text-center text-slate-400 text-xs">No students enrolled in this classroom yet.</td></tr>
        )}
      </tbody>
    </table>
  </div>
  ...
  return (
    <div className="bg-white p-8 rounded-3xl border border-slate-200/80 shadow-xs max-w-2xl space-y-6">
      <p className="text-xs text-slate-600 leading-relaxed font-medium">
        Define criteria for matching students to this classroom. Every field is optional — leave blank for no constraint.
      </p>

      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">Min Attention Span %</label>
          <input type="number" value={form.minAttentionSpanScore} onChange={(e) => setForm({ ...form, minAttentionSpanScore: e.target.value })} className="w-full bg-[#FAF9F5] border border-slate-200 rounded-2xl px-4 py-2.5 text-slate-800 text-sm focus:outline-none focus:border-emerald-500" />
        </div>
        <div>
          <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">Max Attention Span %</label>
          <input type="number" value={form.maxAttentionSpanScore} onChange={(e) => setForm({ ...form, maxAttentionSpanScore: e.target.value })} className="w-full bg-[#FAF9F5] border border-slate-200 rounded-2xl px-4 py-2.5 text-slate-800 text-sm focus:outline-none focus:border-emerald-500" />
        </div>
        <div>
          <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">Min Assessment Score %</label>
          <input type="number" value={form.minScorePercent} onChange={(e) => setForm({ ...form, minScorePercent: e.target.value })} className="w-full bg-[#FAF9F5] border border-slate-200 rounded-2xl px-4 py-2.5 text-slate-800 text-sm focus:outline-none focus:border-emerald-500" />
        </div>
        <div>
          <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">Max Assessment Score %</label>
          <input type="number" value={form.maxScorePercent} onChange={(e) => setForm({ ...form, maxScorePercent: e.target.value })} className="w-full bg-[#FAF9F5] border border-slate-200 rounded-2xl px-4 py-2.5 text-slate-800 text-sm focus:outline-none focus:border-emerald-500" />
        </div>
      </div>

      <div>
        <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Preferred Learning Modes</label>
