...
export const CustomPlanPlayerPage: React.FC = () => {
  ...
  usePageAudio("Customized Learning Plan", () => {
    if (!currentModule) return "Loading your customized plan.";
    return `${currentModule.title}. ${currentModule.description}. ${currentModule.content?.explanation || ""}`;
  });
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200/80 shadow-xs space-y-6">
              ...
              <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-semibold px-3 py-1 rounded-full bg-sky-50 text-sky-800 border border-sky-200 flex items-center gap-1">
                    {currentModule.contentType === "VISUAL_STORY" && <ImageIcon className="w-3.5 h-3.5" />}
                    {currentModule.contentType === "GAME_CHALLENGE" && <Gamepad2 className="w-3.5 h-3.5" />}
                    {currentModule.contentType === "AUDIO_EXPLANATION" && <Volume2 className="w-3.5 h-3.5" />}
                    {currentModule.contentType === "MIXED_LESSON" && <BookOpen className="w-3.5 h-3.5" />}
              ...
              {/* Explanation Text */}
              <div className="bg-[#FAF9F5] p-5 rounded-2xl border border-slate-200/80 space-y-3">
                ...
                {currentModule.content?.example && (
                  <div className="mt-3 p-3 bg-white rounded-xl border border-slate-200/80 text-xs text-slate-600">
              ...
              {/* Interactive Assessment / Game Challenge Check */}
              {question && (
